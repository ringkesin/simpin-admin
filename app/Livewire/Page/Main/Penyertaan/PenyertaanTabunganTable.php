...
use App\Models\Main\VPenyertaanTabungan;
use App\Traits\MyHelpers;
use App\Traits\MyAlert;

class PenyertaanTabunganTable extends DataTableComponent
{
    protected $model = VPenyertaanTabungan::class;
    ...
    {
        $this->setPrimaryKey('penyertaan_id')
        ->setTableRowUrl(function($row) {
            return route('main.penyertaan.approval', ['id' => $row->penyertaan_id]);
        });
    ...
    {
        return [
            Column::make("ID", "penyertaan_id")
                ->hideIf(true),
            Column::make("Tgl. Penyertaan", "tgl_penyertaan_beautify")
                ->sortable(function(Builder $query, string $direction) {
                    return $query->orderBy('tgl_penyertaan', $direction);
                })
                ->searchable(function($query, $searchTerm) {
                    $query->orWhere('tgl_penyertaan_beautify', 'ilike', "%{$searchTerm}%");
                }),
            ...
            Column::make("Jumlah Penyertaan", "jumlah_beautify")
                ->sortable(function(Builder $query, string $direction) {
                    return $query->orderBy('jumlah', $direction);
                })
                ->searchable(function($query, $searchTerm) {
                    $query->orWhere('jumlah_beautify', 'ilike', "%{$searchTerm}%");
                }),
            ...
            Column::make("Status Approval", "status_penyertaan")
                ->sortable()
                ->searchable(function($query, $searchTerm) {
                    $query->orWhere('status_penyertaan', 'ilike', "%{$searchTerm}%");
                }),
