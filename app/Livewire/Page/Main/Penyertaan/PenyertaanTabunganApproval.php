...
class PenyertaanTabunganApproval extends Component
{
    ...
    public array $buktiTransferFiles = [];
    ...
    public function mount()
    {
        ...
        $this->buktiTransferFiles = $this->data->files
            ->filter(fn ($file) => Storage::disk('kkba_simpin')->exists($file->path_file))
            ->map(fn ($file) => [
                'nama_file' => $file->nama_file,
                'url' => Storage::disk('kkba_simpin')->temporaryUrl($file->path_file, now()->addMinutes(5)),
            ])
            ->values()
            ->all();
